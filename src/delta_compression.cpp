...
void FeatureSample::OdessResemblanceDetect(const rocksdb::Slice &value) {
  uint64_t hash = 0;
  for (size_t i = 0; i < value.size(); ++i) {
    hash = (hash << 1) + rand_nums[static_cast<uint8_t>(value[i])];
    if (!(hash & SAMPLE_MASK)) {
...
void FeatureSample::IndexFeatures(SuperFeaturesSet *sfs) {
  for (int i = 0; i < NUM_SUPER_FEATURE; ++i) {
    ...
    sfs->super_features[i] =
        rocksdb::XXH64(&this->features_[i * group_len],
